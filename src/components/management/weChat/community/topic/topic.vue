      ...
      <el-table
        ...
        <el-table-column width="300" fixed="right" label="操作">
          <template slot-scope="scope">
            ...
            <el-button
              size="mini"
              @click="groupPriority(scope.row.publish,scope.row.id,scope.row.priority)">{{scope.row.priority ?
              '取消推荐':'推荐'}}
            </el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog
      ...
      <div class="popOut-box">
        <el-row v-if="!disabledFlag">
          ...
          <el-col :span="20">
            <el-input v-model="topicName" placeholder="请输入话题名称" :disabled="disabledFlag"></el-input>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="4"><p>所属群组 <span>*</span></p></el-col>
          <el-col :span="20">
            <el-select v-model="popCommunityId" placeholder="请选择群组" :disabled="disabledFlag"
                       @change="handleSelectPopCommunity">
              <el-option
        ...
        <el-row>
          ...
          <el-col :span="20">
            <el-input type="textarea" :rows="6" placeholder="请输入内容" :disabled="disabledFlag"
  ...
  export default {
    ...
    methods: {
      ...
      getTopicList() {
        let self = this;
        let param = {
          page: this.pageIndex, topicName: this.topicNameSearch,
