...
import {getMock} from '@/config/mockConfig';
/*++-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
apiDataFilter的定义
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------++*/
let mockId = {mockId: getMock () || ''};
console.log (mockId);
...
let apiDataFilter = {
  ...
  request({
    ...
    if (method === 'post') {
      ...
      opts.params = data;
      if (method === 'jsonp' && jsonp !== '' && jsonp !== undefined)
        opts.jsonp = jsonp;
    }
    /*++-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        不是生产环境在控制台输出请求log
    ...
    if (this.getEnv () !== 'prod') {
      console.log (
        'http请求(' + apiUrl + ')后端接口数据(' + method + ')：' + JSON.stringify (data)
      );
    }
    /*++-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        发起请求
    ...
    if (method === 'post') {
      Vue.http[method] (apiUrl, data, opts).then (
        res => {
          if (parseInt (res.body.code, 10) === apiConf.successStatusCode) {
            successCallback (res.body);
          } else {
            errorCallback (res && res.body, res);
          }
      ...
      Vue.http[method] (apiUrl, opts).then (
        res => {
          if (parseInt (res.body.code, 10) === apiConf.successStatusCode) {
            successCallback (res.body);
          } else {
            errorCallback (res && res.body && res.body, res);
          }
          loadingInstance.close ();
        },
        res => {
    ...
    if ('status' in res) {
      if (res.status === 700) {
        window.location.href =
          decodeURI (
            `${window.location.protocol}//${window.location.host}/login?callbackUrl=`
